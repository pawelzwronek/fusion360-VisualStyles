...
cmds = {S_CmdId: Cmd(S_CmdId, 'Shaded', '/S', vs.ShadedVisualStyle),
        ...
        SVEO_CmdId: Cmd(SVEO_CmdId, 'Shaded with Visible Edges Only', '/SVEO',
                        vs.ShadedWithVisibleEdgesOnlyVisualStyle),
        ...
        WVEO_CmdId: Cmd(WVEO_CmdId, 'Wireframe with Visible Edges Only', '/WVEO',
                        vs.WireframeWithVisibleEdgesOnlyVisualStyle),
...
if sys.platform == 'win32':
    ...
    configPath = os.path.join(os.path.expanduser('~/Library/Application Support/'), 'Autodesk', 'ApplicationPlugins')
...
def run(context):
    ...
    try:
        ...
        if not ConfigControl:
            ...
            if not lstCmd:
                lstCmd = cmdDefs.addListDefinition(id, cmds[id].name,
                                                   adsk.core.ListControlDisplayTypes.CheckBoxListType)
            ...
            if id != VSS_CmdId:
                ...
                if not control:
                    ...
                    if not cmdDef:
                        ...
                        cmdDef = cmdDefs.addButtonDefinition(cmd.id, cmd.name, tooltip,
                                                             (commandResources + cmd.res) if cmd.res else '')
